...
namespace Lacrima_02
{
    public class TitleScreen : screen
    {
        ...
        public override void Draw(SpriteBatch theBatch)
        {
            ...
            theBatch.Draw(titleUI, new Vector2(380, 0), new Rectangle(0, 0, 550, 250), Color.White);

            if (currentMenu == 1)
            {
                theBatch.Draw(titleUI, new Vector2(560, 280), new Rectangle(50, 515, 245, 60), Color.White);
            }
            else
            {
                theBatch.Draw(titleUI, new Vector2(560, 280), new Rectangle(60, 310, 245, 60), Color.White);
            }

            if (currentMenu == 2)
            {
                theBatch.Draw(titleUI, new Vector2(560, 360), new Rectangle(395, 510, 245, 70), Color.White);
            }
            else
            {
                theBatch.Draw(titleUI, new Vector2(560, 360), new Rectangle(410, 300, 245, 70), Color.White);
            }

            if (currentMenu == 3)
            {
                theBatch.Draw(titleUI, new Vector2(560, 450), new Rectangle(745, 510, 245, 65), Color.White);
            }
            else
            {
                theBatch.Draw(titleUI, new Vector2(560, 450), new Rectangle(765, 295, 245, 65), Color.White);
            }
